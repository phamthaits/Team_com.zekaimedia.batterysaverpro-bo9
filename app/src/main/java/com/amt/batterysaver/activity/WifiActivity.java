...
public class WifiActivity extends AppCompatActivity implements View.OnClickListener {
    ...
    TaskWifiBooster mTaskWifiBooster;
    TaskWifiDetail mTaskWifiDetail;
...
//        rlScan.setVisibility(View.VISIBLE);
        ...
        mTaskWifiBooster = new TaskWifiBooster(this, tvScan, new TaskWifiBooster.OnTaskListListener() {
            @Override
            public void OnResult() {
                mTaskWifiDetail = new TaskWifiDetail(WifiActivity.this, tvScan, wifiManager, new TaskWifiDetail.OnTaskBoostListener() {
                    @Override
                    public void OnResult() {
                        ...
                        ivDoneAnim = AnimationUtils.loadAnimation(WifiActivity.this, R.anim.ic_done_anim);
                        ivDone.startAnimation(ivDoneAnim);
                        rocketImageOut.setImageResource(R.drawable.rocket_12);

                        Animation zoom_out = AnimationUtils.loadAnimation(WifiActivity.this, R.anim.zoom_out);
                        rlDone.startAnimation(zoom_out);
                        mHandler1.postDelayed(r1, 2000);
                    }});
                mTaskWifiDetail.execute();
            }
        });
        mTaskWifiBooster.execute();
    }
    ...
    private void loadResult() {
        Animation slideUp = AnimationUtils.loadAnimation(WifiActivity.this, R.anim.zoom_in);
        rlDone.startAnimation(slideUp);
        rlDone.setVisibility(View.GONE);
        fmResult.setVisibility(View.VISIBLE);
        Animation downtoup = AnimationUtils.loadAnimation(WifiActivity.this, R.anim.downtoup);
        fmResult.startAnimation(downtoup);
        flagExit = true;
        SharePreferenceUtils.getInstance(WifiActivity.this).setOptimizeTime(System.currentTimeMillis());
        mHandler.postDelayed(r, 2000);
    }

    public void cancleUIUPdate() {
        if (this.mTaskWifiBooster != null && this.mTaskWifiBooster.getStatus() == AsyncTask.Status.RUNNING) {
            this.mTaskWifiBooster.cancel(true);
            this.mTaskWifiBooster = null;
        }
        if (this.mTaskWifiBooster != null && this.mTaskWifiBooster.getStatus() == AsyncTask.Status.RUNNING) {
            this.mTaskWifiBooster.cancel(true);
            this.mTaskWifiBooster = null;
